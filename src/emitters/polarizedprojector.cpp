...
MTS_VARIANT class PolarizedProjector final : public Emitter<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(Emitter, m_flags, m_world_transform, m_needs_sample_3)
    MTS_IMPORT_TYPES(Texture)

    PolarizedProjector(const Properties &props) : Base(props) {
    ...
    std::string to_string() const override {
        ...
        oss << "PolarizedProjector[" << std::endl
...
MTS_IMPLEMENT_CLASS_VARIANT(PolarizedProjector, Emitter)
MTS_EXPORT_PLUGIN(PolarizedProjector, "Polarized projection emitter")
